...
export class DiceRoller {
    ...
    static async Push(message, html, data){
        ...
        const diceWithResults = message.getFlag("mutant-year-zero", "dicePool").filter(d => (d.diceType === "base" && ( d.value === 6)) || (d.diceType === "gear" && ( d.value === 6)) || (d.diceType === "skill" && d.value === 6));
        ...
        const baseCount = message.getFlag("mutant-year-zero", "dicePool").filter(d => (d.diceType === "base" && d.value !=6)).length;
        const skillCount = message.getFlag("mutant-year-zero", "dicePool").filter(d => d.diceType === "skill" && d.value !=6).length;
        const gearCount = message.getFlag("mutant-year-zero", "dicePool").filter(d => d.diceType === "gear" && d.value !=6).length;
